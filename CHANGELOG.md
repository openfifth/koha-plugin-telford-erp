...
- `configure.tt`: the example output block is now a bordered `page-section` card with a fixed monospace font and no line-wrapping, so the fixed-width columns stay aligned instead of wrapping and losing their column positions; trailing padding is preserved with non-breaking spaces so it survives editors/hooks that trim trailing whitespace
...
[Unreleased]: https://github.com/openfifth/koha-plugin-telford-erp/compare/v0.1.4...HEAD
[0.1.4]: https://github.com/openfifth/koha-plugin-telford-erp/compare/v0.1.3...v0.1.4
