...
- **`voucher_date`** is `aqinvoices.billingdate`, and **`apar_id`** is `aqbooksellers.accountnumber` — both are native Koha fields, not plugin config. Both happen to be unpopulated on every live invoice/vendor in the copy checked on 2026-09-17. **Revised 2026-09-17**: this is a Koha data-entry gap on Telford's side (no invoices with billing dates recorded yet; vendor account numbers never configured), not something the plugin should paper over with a mapping or a fallback date. `generate_batch()` now reads both fields directly and **skips** (logs a warning, leaves unsubmitted) any invoice missing either one, rather than guessing — see `docs/DATA_REQUIREMENTS.md`. It will pick itself back up automatically once Telford fills in the missing Koha data.
...
5. ✅ **Data extraction** — done: `generate_batch()` in `Telford.pm` walks `Koha::Acquisition::Invoices` (closed, date-ranged, not-yet-submitted) → `aqorders` + `Koha::Acquisition::Invoice::Adjustments`, resolving `subcc` via the fund mapping and `apar_id`/`voucher_date` straight from Koha's own vendor/invoice fields (see "Resolved questions" above) and `tax_code` via `Format::tax_code_for_rate`.
6. ✅ **Batch header aggregation** — done: `num_trans`/`total_debits`/`total_credits` computed alongside the line loop in `generate_batch()`.
7. ✅ **Reuse the house infrastructure**: `cronjob_nightly()` (day-of-week schedule, same window logic as the siblings), two-step manual `report()`/`report-step1.tt`/`report-step2.tt`, `tool()`/`manage-submissions.tt`, `_deliver_batch()` ((S)FTP via `Koha::File::Transports`, or local `output/` file), `configure()` extended with delivery/transport/schedule. **Not done**: REST `UploadController` (skipped this pass — cron + manual report cover the actual delivery need; can be added later if an external trigger is wanted).
8. ✅ **Tests** — `t/01-format.t` covers the fixed-width writer and batch-ID rollover, runs on the host with plain `prove` (no KTD needed). Still needed: a DB-backed test for `generate_batch()`/`cronjob_nightly()`/`report_step2()` (needs KTD/fixtures, per `t/01-oracle-integration.t`'s pattern) — verified manually against `o5th25` instead (see below).
...
10. ✅ **Validate against KTD** — done manually against `o5th25`'s live data copy: `generate_batch()`, `cronjob_nightly()`, `report_step1/2()`, `tool()`/`manage_submissions()` and `configure()` all exercised end-to-end (including a real delivered file, byte-checked against the spec's field layout). All test mutations made to `o5th25`'s DB were reverted afterwards.

**Bug found and fixed along the way**: the original scaffold (and this plugin, copying it) put `.tt` templates in a `templates/` subdirectory. `Koha::Plugins::Base::get_template` (via `Module::Bundled::Files::mbf_path`) only ever looks in the module's *own* directory, never a subdirectory — confirmed by checking where the sibling plugins actually keep theirs (flat, no `templates/`). Nothing under `templates/` was ever reachable; moved all four `.tt` files up a level to fix it.
